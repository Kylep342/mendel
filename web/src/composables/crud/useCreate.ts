...
export function useCreate<T>(apiPath: string) {
   ...
   * @template CRUDable The type of the payload being sent.
   * @param {CRUDable} payload The data for the new item to be created.
   * @returns {Promise<T | null>} A promise that resolves with the created item data, or null on failure.
   */
  const create = async <CRUDable>(payload: CRUDable): Promise<T | null> => {
